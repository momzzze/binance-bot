...
async function checkTrailingStop(
  ...
  // Only update stop loss if new trailing stop is higher than current stop loss
  ...
  if (trailingStopPrice > currentStopLoss) {
    log.info(
      `📈 Trailing stop updated for ${symbol}: ` +
        `${currentStopLoss.toFixed(2)} → ${trailingStopPrice.toFixed(2)} ` +
        `(High: ${highestPriceValue.toFixed(2)}, Current: ${currentPrice.toFixed(2)})`
    );
    await updatePositionStopLoss(position.id, trailingStopPrice);
  }
}
